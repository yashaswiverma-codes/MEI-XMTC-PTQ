...
def split_label_score(predicted, num_samples, top):
    ...
    top_labels = np.zeros((num_samples, top), dtype=int)
    top_scores = np.zeros((num_samples, top))
    for i, sample in enumerate(predicted[1:]):
        labels = np.zeros(top, dtype=int)
        scores = np.zeros(top)
...
def read_all(data_path, out_path):
    ...
    for i in range(num_samples):
        ...
        predict_sample = predict_sample.strip(); 
        if not predict_sample: continue # remove the last white space!
...
def precision(pred_mat, true_mat, k):
    ...
    correct_count = np.zeros(k, dtype=int)
    for pred, tr in zip(pred_mat, true_mat):
        tr = np.array(tr)
        match= np.isin(np.array(pred), tr, assume_unique=True)
